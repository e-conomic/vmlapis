...
/*
Package ssn is a reverse proxy.
...
// RegisterSsnServiceHandlerFromEndpoint is same as RegisterSsnServiceHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterSsnServiceHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	...
	return RegisterSsnServiceHandler(ctx, mux, conn)
}

// RegisterSsnServiceHandler registers the http handlers for service SsnService to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterSsnServiceHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterSsnServiceHandlerClient(ctx, mux, NewSsnServiceClient(conn))
}

// RegisterSsnServiceHandlerClient registers the http handlers for service SsnService
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "SsnServiceClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "SsnServiceClient"
